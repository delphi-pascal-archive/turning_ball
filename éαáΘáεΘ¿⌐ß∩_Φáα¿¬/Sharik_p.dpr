...
const
q=80; //чем больше тем медленее и плавнее
dir=3;//скорость изменени€ размеров
var
ce:tCanvas;
s :pbytearray;
mb:array[1..q] of Graphics.tBitmap;
m :array[0..1200,0..1600{Ёто макс.размер экрана},1..3] of byte;
mk:array[0..1024,1..768] of record x,y,t:smallInt end;
ir,dx,dy,yc,xc,q256_sy,q256,kx,ky,p,i,j,sx,y,x,z,h,w,sch,scw,
xr,x1,x2,y1,y2,t,x3,yr,he2,we2,h2,w2:integer;
...
sx:=0;
ir:=256; //размер шарика
for i:=1 to q do begin
inc(sx,scw div q);//убрать чтоб не крутилс€
...
if i>q div 2 then inc(ir,dir) else dec(ir,dir);//изменение размеров

for y:=1 to h-1 do begin
  s:=mb[i].ScanLine[y]; p:=8*3;
  ky:=h2+((y-h2) shl 8 div q256)*256 div ir;
//  ky:=h2+(y-h2) shl 8 div(q256-sy);
  q256_sy:=q256;
  if (ky>0)and(ky<h) then
  for x:=1 to w-1 do begin
    kx:=w2+((x-w2) shl 8 div q256_sy)*256 div ir;
//    kx:=w2+(x-w2) shl 8 div q256_sy;
    if (kx>0)and(kx<w) then begin
    t:=mk[kx,ky].t;
    if t<>-1 then begin
      xr:=mk[kx,ky].x-sx;   if xr<0 then inc(xr,scw);
      yr:=mk[kx,ky].y; if yr<0 then inc(yr,sch);
...
for i:=1 to 300 do begin
  GetCursorPos(pt);if (pn.x<>pt.x)or(pn.y<>pt.y) then break;
  for j:=1 to q do begin ce.draw(x1,y1,mb[j]); inc(p) end;
end;

ce.draw(0,0,b);
t:=(GetTickCount-t)div 1000;
ce.textOut(x1-20,he2-40,'  рутилась '+intToStr(t)+'c. '+
intToStr(p div t)+' fps   —делай быстрей и красивей! ');
halt;
end.
